...
if __name__ == "__main__":
    ...
    for idx in range(len(result)):
        ...
        st_code = "0"+str(200)
        url = urls[idx]
        if type(result[idx]) == bytes:
            sumf.write(result[idx])
            continue            
        try:
            ## Type: dict
            ## dict keys: html, title, text
            webdict = getWebpageText(url, result[idx])
            webtype = webdict['type']
            if webdict['text'] in ["1001","2000","2001"]:
                st_code = webdict['text']
                sumf.write(statusCodeWriter(st_code, url, webtype))
                continue
            ...
            newline_m = ["webpage", col_id, col_event, "EFC", "EFC"]
            ...
            st_code = "2001"
            sumf.write(statusCodeWriter(st_code, url))
            print(e)
    ## End time
    counter = len(urls)
    timeend = time.time()
    time_diff = timeend - timestart
    log_name = "crawling_%s" %time.time()
