...
def getPOS_rdd(token, tagger):    
    POS = tagger([token])
    return ["{}:{}".format(i,j) for i,j in POS][0]
"""
Input: tokenized list
...
def getStemmed_rdd(token, stemmer):
    allTokens_stem = stemmer.stem(token)
    return allTokens_stem
...
def getLemmatized_rdd(token, wordnet_lemmatizer):

    allTokens_lemmatize = wordnet_lemmatizer.lemmatize(token)
    return allTokens_lemmatize  
"""
Input: tokenized list
Output: list of NERs
...
if __name__ == "__main__":
    ...
    with open(sys.argv[1]) as f:
        for url in f:
            ...
            try:
                ...
                """Spark rdd"""
                rddtokens = sc.parallelize(webtokens)
                
                try:
                    webPOS = getPOS(webtokens)
                    """Potential Spark mark since the input is a list"""
                    #rddPOS = sc.parallelize(webtokens)
                    # To run: 
                    ...
                    #webPOS = rddPOS.map(lambda x: getPOSrdd(x)).collect()
                    #print(webPOS)
                except:
                    webPOS = [""]
                ## Stemming and lemmatization
                """Spark mark"""
                webstem = rddtokens.map(lambda x: getStemmed_rdd(x, stemmer)).collect()
                #print(webstem)
    ...
    ## End time
    timeend = time.time()
    time_diff = float(timeend) - float(timestart)
    log_name = "stemming_%s" %time.time()
    cmd = "echo url size: %s time: %s > log_%s.txt" %(counter, time_diff, log_name)
    os.system(cmd)
